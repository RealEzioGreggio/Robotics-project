...
extern float pos_rad_r;
extern float pos_rad_l;
...
void TIM6_DAC_IRQHandler(void)
{
  ...
  pos_rad_r = (encoder_get_position_rad(&encoder_r)) / MOTOR_GEAR_REDUCTION;
  ...
  //pos_rads_l = (encoder_get_velocity_rads(&encoder_l)) / MOTOR_GEAR_REDUCTION;
/*
  if(vel_ref_r > 0){

	  pid_limits(&pid_r, 0.0, 1.0);
   }else if (vel_ref_r < 0) {

	   pid_limits(&pid_r, -1.0, 0.0);
   } else {
	   pid_limits(&pid_r, 0.0, 0.0);
   }
*/
  ...
  len = snprintf(msg, sizeof(msg), "r%f,pr%f\n\r", velocity_rads_r, pos_rad_r);
